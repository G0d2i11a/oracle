...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    ...
    `${colors.bullet("•")} Oracle is one-shot by default. API runs can continue with ${colors.accent("--followup <sessionId|responseId>")}; browser runs can continue a stored ChatGPT session with ${colors.accent("--engine browser --followup <sessionId>")} or add sequential turns with repeated ${colors.accent("--browser-follow-up")}.`,
