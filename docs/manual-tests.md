...
- Heartbeats: Browser runs emit `--heartbeat` status while waiting. Long Pro Extended / explicitly requested Thinking Heavy runs should show `[browser] ChatGPT response progress ...` or `[browser] Waiting for ChatGPT response ...`; the log must not include reasoning text from the side panel.
